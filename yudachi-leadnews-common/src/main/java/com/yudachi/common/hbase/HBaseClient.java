...
public class HBaseClient {
    ...
    public boolean creatTable(String tableName, List<String> columnFamily) {
        ...
        try {
            ...
            columnFamily.forEach(cf -> {
                familyDescriptors.add(new HColumnDescriptor(Bytes.toBytes(cf)));
            });

            HTableDescriptor tableDescriptor = new HTableDescriptor(TableName.valueOf(tableName));
           for (HColumnDescriptor hColumnDescriptor : familyDescriptors){
    ...
    public List<String> getAllFamilyNames(String tableName) {
        HColumnDescriptor[] familyDescriptorList = null;
        List<String> familyNameList = new ArrayList<>();
        try {
            Table table = getTable(tableName);
            familyDescriptorList = table.getTableDescriptor().getColumnFamilies();
        ...
        if (null != familyDescriptorList && familyDescriptorList.length > 0) {
            familyNameList = Arrays.stream(familyDescriptorList).map(HColumnDescriptor::getNameAsString).collect(Collectors.toList());
        }
